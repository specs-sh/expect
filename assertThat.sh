...
{ 
    ...
    local EXPECT_ACTUAL= EXPECT_MATCHER= EXPECT_NOT= EXPECT_ACTUAL_IS_ARRAY_NAME= EXPECT_ACTUAL_ARRAY_NAME= EXPECT_BLOCK_OPEN= EXPECT_BLOCK_CLOSE= EXPECT_COMMAND_STDOUT= EXPECT_COMMAND_STDERR= EXPECT_BASH_ASSOCIATIVE_ARRAYS= EXPECT_BASH_NAME_REFERENCES= __expect__argument= __expect__isCommand= __expect__runCommandInSubShell= __expect__stdoutTempFile= __expect__stderrTempFile= __expect__nounsetOn= __expect__returnValue=;
    ...
    case "$1" in 
        ...
        {{)
            EXPECT_BLOCK_OPEN={{;
            EXPECT_BLOCK_CLOSE=}};
            __expect__runCommandInSubShell=true
        ;;
        {{{)
            EXPECT_BLOCK_OPEN={{{;
            EXPECT_BLOCK_CLOSE=}}}
        ...
        [)
            EXPECT_BLOCK_OPEN=[;
            EXPECT_BLOCK_CLOSE=];
            EXPECT_ACTUAL=EXPECT_ACTUAL_ARRAY;
            EXPECT_ACTUAL_IS_ARRAY_NAME=true;
            EXPECT_ACTUAL_ARRAY_NAME=EXPECT_ACTUAL_ARRAY
        ;;
        [[)
            EXPECT_BLOCK_OPEN=[[;
            EXPECT_BLOCK_CLOSE=]];
    ...
    if [ -n "$EXPECT_BLOCK_OPEN" ]; then
        for __expect__argument in "${EXPECT_ORIGINAL_ARGUMENTS[@]:2}";
        do
            [ "$__expect__argument" = "$EXPECT_BLOCK_CLOSE" ] && { 
                __expect__isCommand=true;
                break
            };
    ...
    if [ "$__expect__isCommand" = true ] && [ "$EXPECT_ACTUAL_IS_ARRAY_NAME" != true ]; then
        __expect__stderrTempFile="$( mktemp )" || { 
            echo "run: failed to create temporary file to store standard error using 'mktemp'" 1>&2;
            return 2
        };
        if [ "$__expect__runCommandInSubShell" = true ]; then
            ...
            __expect__stdoutTempFile="$( mktemp )" || { 
                echo "run: failed to create temporary file to store standard output using 'mktemp'" 1>&2;
                return 2
            };
            "${EXPECT_COMMAND[@]}" > "$__expect__stdoutTempFile" 2> "$__expect__stderrTempFile" && EXPECT_COMMAND_EXITCODE=$? || EXPECT_COMMAND_EXITCODE=$?;
            EXPECT_COMMAND_STDOUT="$( < "$__expect__stdoutTempFile" )" || echo "run: failed to read standard output from temporary file '$__expect__stdoutTempFile' created using 'mktemp'" 1>&2;
            [ -f "$__expect__stdoutTempFile" ] && { 
                rm "$__expect__stdoutTempFile" || echo "run: failed to delete temporary file used for standard output '$__expect__stdoutTempFile' created using 'mktemp'" 1>&2
            };
        fi;
        EXPECT_COMMAND_STDERR="$( < "$__expect__stderrTempFile" )" || echo "run: failed to read standard error from temporary file '$__expect__stderrTempFile' created using 'mktemp'" 1>&2;
        [ -f "$__expect__stderrTempFile" ] && { 
            rm "$__expect__stderrTempFile" || echo "run: failed to delete temporary file used for standard error '$__expect__stderrTempFile' created using 'mktemp'" 1>&2
        };
        __expect__stdoutTempFile= __expect__stderrTempFile=;
        EXPECT_ACTUAL="${EXPECT_COMMAND_STDOUT}${EXPECT_COMMAND_STDERR}";
    else
        if [ -z "$EXPECT_ACTUAL" ]; then
            EXPECT_ACTUAL="$1";
            shift;
        fi;
    fi;
    ...
    } && EXPECT_BASH_NAME_REFERENCES=true;
    EXPECT_ARGUMENTS=("$@");
    EXPECT_SEARCH_ARGUMENTS=("${EXPECT_ARGUMENTS[@]}");
    EXPECT_MATCHER=;
    shopt -qo nounset && { 
        __expect__nounsetOn=true;
        set +u
    };
    until (( ${#EXPECT_ARGUMENTS[@]} == 0 )); do
        if Expect.core.nextMatcher; then
            "$EXPECT_MATCHER" "${EXPECT_ARGUMENTS[@]}" || { 
                __expect__returnValue=$?;
                [ "$__expect__nounsetOn" = true ] && set -u;
                return $__expect__returnValue
            };
            EXPECT_MATCHER= EXPECT_NOT=;
        else
            echo "No matcher found for arguments: ${EXPECT_SEARCH_ARGUMENTS[*]}" 1>&2;
            [ "$__expect__nounsetOn" = true ] && set -u;
            return 44;
        fi;
        EXPECT_SEARCH_ARGUMENTS=("${EXPECT_ARGUMENTS[@]}");
...
{ 
    ...
    until (( ${#EXPECT_ARGUMENTS[@]} == 0 )) || declare -F "$EXPECT_MATCHER" &> /dev/null; do
        case "${EXPECT_ARGUMENTS[0]}" in 
            ...
            ;;
            *)
                EXPECT_MATCHER+=".${EXPECT_ARGUMENTS[0]}";
                EXPECT_ARGUMENTS=("${EXPECT_ARGUMENTS[@]:1}")
            ;;
        esac;
...
{ 
    case "${EXPECT_INSPECT:-declare}" in 
        ...
        ;;
        *)
            echo "Unknown EXPECT_INSPECT value '${EXPECT_INSPECT:-}', expected one of: declare, cat, simple" 1>&2;
            return 1
        ;;
    esac
